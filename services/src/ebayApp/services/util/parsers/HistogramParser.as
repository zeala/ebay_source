package ebayApp.services.util.parsers
{
	...
	import dto.histograms.AspectValueHistogram;
	import dto.histograms.CategoryHistogram;
	import dto.histograms.CategoryHistogramContainer;
	import dto.histograms.ConditionHistogram;
	import dto.histograms.ConditionHistogramContainer;
	...
	public class HistogramParser
	{
		...
		public static function parseCategoryHistogramContainer( xmlData : XMLList) : CategoryHistogramContainer
		{
			...
			if ( histograms.length() > 0 )
			{
				container.categoryHistograms = new ArrayCollection();
				for ( var i:int = 0; i < histograms.length() ; i++)
				{
					var histogram : CategoryHistogram = parseCategoryHistogram( histograms[i]);
					container.categoryHistograms.addItem(histogram);
				}
			}
